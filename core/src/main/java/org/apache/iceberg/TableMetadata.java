...
public class TableMetadata implements Serializable {
  ...
  static TableMetadata newTableMetadata(Schema schema,
    ...
    for (SortField sortField : sortOrder.fields()) {
      BoundTerm<?> term = sortField.term();
      if (term instanceof BoundReference) {
        // lookup the reference name and bind it using the new schema
        NamedReference<?> newTerm = Expressions.ref(term.ref().field().name());
        sortOrderBuilder.orderBy(newTerm, sortField.direction(), sortField.nullOrder());
      } else if (term instanceof BoundTransform) {
        // look up the source field name and bind the transform using the new schema
        Transform<?, ?> transform = ((BoundTransform<?, ?>) term).transform();
        UnboundTerm<?> newTerm = Expressions.transform(term.ref().field().name(), transform);
        sortOrderBuilder.orderBy(newTerm, sortField.direction(), sortField.nullOrder());
      } else {
    ...
    return new TableMetadata(null, formatVersion, UUID.randomUUID().toString(), location,
        ...
        INITIAL_SORT_ORDER_ID, ImmutableList.of(freshSortOrder),
  ...
  TableMetadata(InputFile file,
    ...
    // TODO: figure out whether we want to be consistent here
