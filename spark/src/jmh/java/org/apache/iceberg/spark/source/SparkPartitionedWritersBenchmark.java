...
import org.apache.iceberg.spark.source.SparkWrite.PartitionedDataWriter;
...
public class SparkPartitionedWritersBenchmark {
  ...
  public void writeUsingOldPartitionedWriter() throws IOException {
    ...
    try (SparkPartitionedWriter closableWriter = new SparkPartitionedWriter(
  ...
  public void writeUsingNewPartitionedWriter() throws IOException {
    ...
    try (PartitionedDataWriter writer = new PartitionedDataWriter(taskWriter, table.spec(), dataSchema, dataSparkType)) {
      for (InternalRow row : rows) {
        writer.write(row);
      }
    }
